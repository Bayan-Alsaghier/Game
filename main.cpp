...
double green[3][3] = {0,250,12,0,330,12,0,360,12};
int ei = 1; // enemy index
int gr = 1; // green index
...
int main(){
	...
	while(1){
		...
		for(int i=0; i<ei; i++){
			...
			circle(green[i][0],green[i][1],13);
			
			// update green position
			green[i][0] += green[i][2];
			
			// if green balls goes beyond screen reset thier position
			if( green[i][0] >= 1000 ){
				green[i][0] = 0; 
				green[i][2] = 8+rand()%8;
			}
			
			// if first green ball goes beyond 300px from left generate next ball
			if( green[i][0] >= 300 && green[i][0] < 310 )
				if( gr < 3 )
					gr++;
		}
...
int GreenHitPalyer(){
	for(int i=0; i< gr; i++){
		if( green[i][0]+40 >= px-pSize && green[i][0] <= px+pSize ){
				if( py+pSize >= green[i][1] && py <= green[i][1] ){
					return 1;
				}
		}
	}
